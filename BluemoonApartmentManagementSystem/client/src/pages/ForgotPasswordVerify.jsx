    ...
    <div style={containerStyle}>
      <div style={{ ...gridStyle, flexDirection: isMobile ? 'column' : 'row' }}>
        <div style={{ ...formSideStyle, padding: isMobile ? '40px 20px' : '80px' }}>
          <h2 style={{ fontSize: isMobile ? '28px' : '36px', marginBottom: '10px' }}>Verify Code</h2>
          <p style={{ marginBottom: '40px', opacity: 0.7 }}>Nhập mã số xác thực đã được gửi:</p>
          
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '25px' }}>
            <input type="text" placeholder="Code (e.g. 123456)" required style={inputStyle} />
            <button type="submit" style={{ ...btnStyle, background: '#2e8b57' }}>Verify</button>
